-- תיקון סופי: הוספת שליחת התראות לטבלת notifications + פוש נוטיפיקציה

-- 1. תיקון הפונקציה create_simple_notification
...
BEGIN
    ...
    -- Determine content type and notification type from table name
    CASE TG_TABLE_NAME
        ...
        WHEN 'music_albums' THEN
            ...
            v_action_url := '/music/' || NEW.id::text;
        ELSE
    ...
    -- 1. Create notification record for ALL active users
...
-- 2. תיקון הפונקציה notify_new_message
...
BEGIN
  ...
  LOOP
    -- 1. Create notification record for each recipient
    INSERT INTO notifications (
      user_id,
      title_he,
      content_he,
  ...
  -- 2. Get FCM tokens for push notification
  ...
  -- 3. Send push notification if there are tokens
